...
def glob_match(pattern: str, value: str) -> bool:
    # very small glob matcher: '*' wildcard only
    if pattern == value:
        return True
    parts = pattern.split("*")
    if len(parts) == 1:
...
def is_allowlisted(owner_repo: str, policy: Dict[str, Any]) -> bool:
    ...
    # future: "allowlist_or_popular"
    return (allowed and not denied)


def skill_covers_capability(skill: Dict[str, Any], cap: str, capdef: Dict[str, Any]) -> bool:
    # If skill declares explicit capabilities, treat them as authoritative.
    declared = skill.get("capabilities") or []
    if declared:
        return cap in declared
    # Otherwise fall back to naive keyword matching.
    hay = f"{skill.get('name','')} {skill.get('description','')}".lower()
    kws = (capdef.get("keywords") or [cap])
...
def compute_coverage(manifest: Dict[str, Any], caps: List[str], capdefs: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for cap in caps:
        ...
        hits = [s for s in skills if skill_covers_capability(s, cap, capdef)]
        covered[cap] = hits
    return covered


def run_skills_find(query: str) -> str:
    # Use npx to avoid assuming global install of skills CLI
    proc = subprocess.run(
...
def main() -> None:
    ...
    candidates: Dict[str, Any] = {"generated_at": state["updated_at"], "caps": caps, "missing_caps": missing, "by_cap": {}}
