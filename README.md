...
## Shared network для “инфры” (ClickHouse, etc.)

Зачем: иногда нужно, чтобы песочница видела сервисы (например `clickhouse`) **по DNS имени контейнера**, а не через проброс портов/`host.docker.internal`. Для этого песочница подключается к внешней сети `infra` (используется в профиле 1С: `.devcontainer/docker-compose.onec.yml`).

Как это работает:

- Контейнер `agent` остаётся в своей default-сети devcontainer (для обычного доступа наружу).
- Дополнительно он подключён к external-сети `infra`, куда ты подключаешь только нужные сервисы.

Команды:

- Создать сеть один раз:
  - `docker network create infra`
- Если хочешь другое имя сети:
  - создай сеть с нужным именем и выставь переменную `AGENT_INFRA_NETWORK` (в окружении Docker/Dev Containers) на это имя.

Как подключить сервисы к `infra`:

- **В сервисе** (`services.<name>.networks`) указываешь **список** сетей, к которым он должен быть подключён.
- **В конце файла** (`networks:`) объявляешь саму сеть `infra` как external.

Пример (если у сервиса уже есть своя сеть `internal`):
...
Пример (если `networks:` уже есть, просто дописываешь `infra` туда же):
...
Антипример (так делать нельзя — это ломает compose, сеть объявляется не там):
...
Альтернатива (быстро, но хуже воспроизводимость): вручную подключить уже запущенный контейнер к сети:
